...
class e03_collections extends HandsOnSuite {
  ...
  section("Map") {
    ...
    exercice("manipuler") {
      countries.get("fr") shouldBe "France"
      countries.map(country => (country._1, country._2.length)) shouldBe Map("fr" -> 6, "be" -> 8, "en" -> 10)
      countries.map { case (code, name) => (code, name.length) } shouldBe Map("fr" -> 6, "be" -> 8, "en" -> 10) // en utilisant le pattern matching \o/
      countries.filter(_._2.contains("r")) shouldBe Map("fr" -> "France", "be" -> "Belgique")
      countries.find(_._2.contains("e")) shouldBe Some("be" -> "Belgique")
    }
  }
  ...
  section("API collection") {
    ...
    exercice("groupBy") {
      ...
      words.groupBy(_.length) shouldBe Map(5 -> List("table", "chaise"), 6 -> List("bureau", "écran"), 10 -> List("ordinateur"))
      words.groupBy(_.contains('c'))(true) shouldBe List("chaise")
      nums.groupBy(_ % 2 == 0)(true) shouldBe List(6, 2)
    }
    ...
    exercice("flatMap") {
      ...
      Some("2").map(toInt) shouldBe Some(2)
      Some("2").flatMap(toInt) shouldBe 2
      Some("a").map(toInt) shouldBe None
      Some("a").flatMap(toInt) shouldBe None
      None.map(toInt) shouldBe None
      None.flatMap(toInt) shouldBe None

      val nums = List("1", "foo", "3")
      nums.map(toInt) shouldBe  List(1, -1, 3)
    ...
    exercice("fold") {
      ...
      words.fold("")(_ + _.length.toString) shouldBe "134"
    }
  ...
  section("Mise en pratique") {
    ...
    val (speakers: List[Speaker], talks: List[Talk], rooms: List[Room], slots: List[Slot]) = Await.result(DevoxxApi.getModel(), 5.seconds)
    ...
    def frenchPercentageOfTalks(): Int = {
      val frenchTalks = talks.count(_.lang == "FR")
      val totalTalks = talks.length
      (frenchTalks * 100) / totalTalks
    }

    def fetchSpeakerTalks(id: SpeakerId): List[Talk] = {
      talks.filter(_.speakers.exists(_.talkId == id))
    }

    def roomSchedule(id: RoomId): List[(Long, Long, TalkId)] = {
      slots.filter(_.roomId == id).map(slot => (slot.start, slot.end, slot.talkId))
    }
